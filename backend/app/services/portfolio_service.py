"""
Portfolio business logic — backed by Groww holdings/positions with simulation fallback.
...
in app.api.portfolio. app.api.portfolio is now a thin router that parses
requests and delegates here.

A few names defined in this module are imported directly by other modules —
app.api.portfolio re-exports them for backward compatibility, so do not
rename/remove them without also updating those call sites:
  - app.api.delivery_paper: _yahoo_quote_map, optimize_portfolio, _build_themes
"""
...
async def _yahoo_quote(client: httpx.AsyncClient, symbol: str) -> dict | None:
    ...
    try:
        ...
        logger.debug("Yahoo quote fetch failed for %s", symbol, exc_info=True)
        return None
                    ...
                    try:
                        ...
                        if missing:
                            try:
                                ...
                                logger.warning(
                                    "BSE LTP fallback fetch failed for holdings",
                                    exc_info=True,
                                    extra={"log_type": "portfolio_event", "event": "ltp_bse_fallback_error", "symbols": missing},
                                )
...
async def _daily_series(client: httpx.AsyncClient, symbol: str) -> dict | None:
    try:
        ...
        logger.debug("Yahoo daily series fetch failed for %s", symbol, exc_info=True)
        return None
...
async def _ai_opportunities(held: set) -> list[dict]:
    ...
    try:
        ...
        logger.warning("Failed to load AI watchlist for opportunities", exc_info=True)
        items = []
    out = []
    for it in items:
...
def _parse_llm_json(text: str | None) -> dict | None:
    ...
    try:
        ...
        logger.debug("Failed to parse LLM optimize JSON response", exc_info=True)
        return None
...
def _enrich_actions(plan: dict, stocks: list[dict], signals: dict, opps: list[dict], total_value: float) -> None:
    ...
    try:
        ...
        logger.warning("stocks_master import failed while enriching trade actions; exchange routing defaults to NSE", exc_info=True)
        STOCKS_BY_SYMBOL = {}
...
async def _latest_scan_version() -> str | None:
    ...
    try:
        ...
        logger.debug("Failed to read latest scan version from cache", exc_info=True)
        return None
...
async def optimize_portfolio(use_llm: bool = True, refresh: bool = False):
    ...
    if not refresh:
        try:
            ...
            logger.warning("Failed to read cached optimization plan; recomputing", exc_info=True)
    ...
    try:
        ...
        logger.warning("stocks_master import failed while sector-enriching optimize input; defaulting to 'Other'", exc_info=True)
        for s in stocks:
    ...
    try:
        ...
        logger.warning("Failed to persist optimization plan to cache", exc_info=True)
...
async def _invest_candidates(max_stocks: int) -> list[dict]:
    ...
    try:
        ...
        logger.warning("Failed to load AI watchlist for invest candidates", exc_info=True)
        data = {}
    pool: dict = {}
...
async def invest_plan(amount: float, max_stocks: int = 6):
    ...
    try:
        ...
        logger.warning("stocks_master import failed while building invest-plan orders; exchange routing defaults to NSE", exc_info=True)
        STOCKS_BY_SYMBOL = {}
...
async def _ranked_items() -> list[dict]:
    ...
    try:
        ...
        logger.warning("Failed to load AI ranked board from cache", exc_info=True)
        items = []
    for it in items:
...
async def _build_baskets() -> list[dict]:
    ...
    try:
        ...
        logger.warning("Failed to load committed watchlist for high-conviction basket", exc_info=True)
        comm = []
    hc = []
    for it in comm:
...
async def fund_basket_invest(basket: str, amount: float):
    ...
    try:
        ...
        logger.warning("stocks_master import failed while building basket-invest orders; exchange routing defaults to NSE", exc_info=True)
        STOCKS_BY_SYMBOL = {}
...
async def _theme_board() -> dict:
    ...
    if missing:
        try:
            ...
            logger.debug("Yahoo quote fetch failed for theme seed symbols", exc_info=True)
            qm = {}
        for s in missing:
...
async def _build_themes() -> list[dict]:
    ...
    try:
        ...
        logger.debug("Failed to read cached themes; recomputing", exc_info=True)
    await ensure_loaded()
    ...
    try:
        ...
        logger.debug("Failed to persist themes cache", exc_info=True)
    return out
...
async def theme_analytics(theme_id: str):
    ...
    try:
        ...
        logger.debug("Failed to read cached theme analytics for %s; recomputing", theme_id, exc_info=True)
    baskets = await _build_themes()
    ...
    try:
        ...
        logger.debug("Failed to persist theme analytics cache for %s", theme_id, exc_info=True)
    return {"status": "success", "data": data}
...
async def _yahoo_returns(client: httpx.AsyncClient, ysym: str) -> dict | None:
    ...
    try:
        ...
        logger.debug("Yahoo returns fetch failed for %s", ysym, exc_info=True)
        return None
    if len(closes) < 30:
        return None
    last = closes[-1]
...
async def _yh_full(client: httpx.AsyncClient, ysym: str) -> dict | None:
    ...
    try:
        ...
        logger.debug("Yahoo full-chart fetch failed for %s", ysym, exc_info=True)
        return None
