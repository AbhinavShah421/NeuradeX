...
const GbmTrainerCard: React.FC<GbmTrainerCardProps> = ({ models, training, trainMsg, trainGbm }) => {
  ...
  return (
    <div className="nd-pm-card" style={{ borderLeft: '3px solid #34d399' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
        <span className="material-icons" style={{ fontSize: 18, color: '#34d399' }}>account_tree</span>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        <button
          ...
          onClick={trainGbm}
          disabled={training}
          style={{ borderRadius: 9, padding: '9px 18px', fontSize: 13, fontWeight: 600, gap: 7, background: '#34d399', borderColor: '#34d399' }}
        >
        ...
        {trainMsg && (
          <span style={{ fontSize: 12, color: trainMsg.ok ? 'var(--nd-green)' : '#fb5c7d', lineHeight: 1.5 }}>
            {trainMsg.text}
          </span>
        )}
      </div>
