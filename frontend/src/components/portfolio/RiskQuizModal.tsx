  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: '#00000080', zIndex: 1200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--nd-surface)', border: '1px solid var(--nd-border)', borderRadius: 14, width: '100%', maxWidth: 460, maxHeight: '88vh', overflow: 'auto', padding: 22 }}>
        <div className="nd-section-title" style={{ fontSize: 13, marginBottom: 4 }}>Find your risk profile</div>
        ...
        {(() => { const total = quizAns.reduce((s, v) => s + v, 0); const risk = riskFromScore(total);
          return (
            <div style={{ marginTop: 8, padding: '10px 12px', background: 'var(--nd-surface)', borderRadius: 8, fontSize: 12.5 }}>
              ...
              <button className="nd-btn nd-btn-primary" style={{ float: 'right', padding: '5px 13px' }}
                onClick={() => { setPlanForm({ ...planForm, risk }); onClose(); }}>Use this</button>
            </div>
          ); })()}
      </div>
    </div>
