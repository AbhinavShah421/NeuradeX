...
const LiveSessions: React.FC = () => {
  ...
  // New-session form
  const [mode, setMode] = useState<'replay' | 'paper'>('replay');
  ...
  // Poll list every 3s; poll selected detail every 2.5s — this is what makes a
  // background session reappear after a refresh and keep updating live.
  ...
  const startSession = async () => {
    ...
    try {
      ...
      await loadList();
      setSelectedId(d.id);          // open it immediately
    } catch (e: any) {
  ...
  const deleteSession = async (id: string) => {
    try { await apiService.sessionDelete(id); if (selectedId === id) { setSelectedId(null); setDetail(null); } await loadList(); } catch {}
  };
  const changeSpeed = async (id: string, s: number) => { try { await apiService.sessionSpeed(id, s); loadDetail(id); } catch {} };

  // Only running sessions stay on this page; finished ones live in Orders.
  // Keep the currently-open session visible even if it just finished.
  const visibleSessions = sessions.filter(s => s.status === 'running' || s.id === selectedId);

  const markers: TradeMarker[] = (detail?.tradesList ?? []).map((t: any) => ({
    timestamp: t.timestamp, action: t.action, price: t.price,
  }));
    ...
    <div>
      ...
      <div style={{ ...card, marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-start', gap: 8, marginBottom: 6 }}>
          <span className="material-icons" style={{ color: 'var(--nd-green)', fontSize: 20, lineHeight: 1 }}>monitoring</span>
          <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: 'var(--nd-text-1)' }}>Live Sessions</h2>
        </div>
        <p style={{ margin: 0, fontSize: 13, lineHeight: 1.6, color: 'var(--nd-text-2)' }}>
          Sessions run on the server with the full 7-agent ensemble. They keep advancing in the
          background, survive a page refresh, run several at once, and can be reopened any time.
        </p>
      </div>
      ...
      <div style={{ ...card, marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 12 }}>
          ...
          <Field label="Symbol" full>
            <StockPicker value={symbol} onChange={(sym) => setSymbol(sym)} />
          </Field>
          {mode === 'replay' && (
            <Field label="Date">
              <input className="nd-input" type="date" value={date} onChange={e => setDate(e.target.value)} style={fieldStyle} />
            </Field>
          )}
          <Field label="Start time">
            <input className="nd-input" type="time" value={startTime} onChange={e => setStartTime(e.target.value)} style={fieldStyle} />
          </Field>
          <Field label="Capital (₹)">
            <input className="nd-input" type="number" inputMode="numeric" value={capital} onChange={e => setCapital(e.target.value)} style={fieldStyle} />
          </Field>
          <Field label="Speed">
      ...
      {/* Running sessions list — completed ones move to Orders */}
      <div style={{ ...card, marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
          <h3 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: 'var(--nd-text-1)' }}>
            Running Sessions <span style={{ color: 'var(--nd-text-3)', fontWeight: 400 }}>({visibleSessions.length})</span>
          </h3>
          <a href="/neuradex/orders" style={{ fontSize: 12, color: 'var(--nd-green)', textDecoration: 'none' }}>
            Completed trades → Orders
          </a>
        </div>
        {visibleSessions.length === 0 ? (
          <div style={{ fontSize: 13, color: 'var(--nd-text-3)' }}>No running sessions — start one above. It keeps running here even if you refresh; once it finishes, its trades appear in <strong>Orders</strong>.</div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(240px,1fr))', gap: 10 }}>
            {visibleSessions.map(s => (
              <div key={s.id} onClick={() => setSelectedId(s.id)}
                style={{ background: selectedId === s.id ? 'var(--nd-bg)' : 'var(--nd-bg)', border: `1px solid ${selectedId === s.id ? 'var(--nd-green)' : 'var(--nd-border)'}`, borderRadius: 10, padding: 12, cursor: 'pointer' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 }}>
                    <StatusDot status={s.status} />
                    <span style={{ fontWeight: 700, color: 'var(--nd-text-1)' }}>{s.symbol}</span>
                    <span style={{ fontSize: 10, color: 'var(--nd-text-3)', textTransform: 'uppercase' }}>{s.mode}</span>
                  </div>
        ...
        <div style={card}>
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 12, color: 'var(--nd-text-3)' }}>{detail.date} · {detail.currentTime} · {detail.mode}</span>
